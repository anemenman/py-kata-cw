...
⚠️ Note: The order of appearance of a clause in a query doesn't matter. However, when it comes time for you to run the
query, you MUST execute the clauses in this logical order: from first, then where, then groupBy, then having, then
select and finally orderBy.
...
query().select(profession).from_(persons).execute() # select receives a function that will be called with the values of
the array
...
orderBy should be called after groupBy, so the values passed to orderBy function are the grouped results by the groupBy
function.
...
# SELECT * FROM numbers GROUP BY parity HAVING odd(number) = true <- I know, this is not a valid SQL statement, but you
...
If any of the unrepeatable clauses are repeated in the query, your solution MUST raise an Error object with the error
message "duplicate " followed by the name of the duplicated clause. If the clause is multi-word, merge it into
one (ex: groupby).

For example, if the groupBy clause is duplicated, you should throw an Error with the exact string message
"duplicate groupby" (capitalization doesn't matter).
For Python, there are Exception classes preloaded to use for this purpose; DuplicateFromError, DuplicateSelectError,
DuplicateGroupByError, and DuplicateOrderByError. You do not need to provide a message to these classes.
"""
...
class QueryBuilder:
    ...
    def select(self, func=None):
        if self._select_called: raise DuplicateSelectError()
        self._select_called = True
        self._select_func = func
        return self

    def from_(self, *tables):
        if self._from_called: raise DuplicateFromError()
        self._from_called = True
        self._from_tables = tables
        return self

    def where(self, *filters):
        self._where_filters.append(filters)
        return self

    def group_by(self, *funcs):
        if self._group_by_called: raise DuplicateGroupByError()
        self._group_by_called = True
        self._group_by_funcs = funcs
        return self

    def having(self, *filters):
        self._having_filters.append(filters)
        return self

    def order_by(self, func):
        if self._order_by_called: raise DuplicateOrderByError()
        self._order_by_called = True
        self._order_by_func = func
        return self

    def execute(self):
        # 1. FROM: Cartesian product
        if not self._from_tables:
            result = []
        elif len(self._from_tables) == 1:
            result = list(self._from_tables[0])
        else:
            result = [list(comb) for comb in product(*self._from_tables)]

        # 2. WHERE: OR within a call, AND between calls
        for filter_group in self._where_filters:
            result = [item for item in result if any(f(item) for f in filter_group)]

        # 3. GROUP BY: recursive grouping
        if self._group_by_funcs:
            result = self._group(result, list(self._group_by_funcs))

        # 4. HAVING: group filtering
        for filter_group in self._having_filters:
            result = [group for group in result if any(f(group) for f in filter_group)]
        ...
        if self._order_by_func:
            result = sorted(result, key=cmp_to_key(self._order_by_func))

        return result

    def _group(self, items, funcs):
        if not funcs or not items:
            return items
        ...
        if remaining_funcs:
            return [[key, self._group(groups[key], remaining_funcs)] for key in key_order]
        else:
            return [[key, groups[key]] for key in key_order]


# numbers = [1, 2, 3]
# print(query().select().from_(numbers[:]).__str__())
...
print(query().select().from_(persons).execute())
...
print(query().select(profession).from_(persons).execute())

numbers = [1, 2, 3]
print(query().select().execute())

print(query().from_(numbers).execute())
...
print(query().select(profession).from_(persons).where(is_teacher).execute())
...
print(query().select(name).from_(persons).where(is_teacher).execute())

# GROUP BY
print(query().select().from_(persons).group_by(profession).execute())

print(query().select().from_(persons).where(is_teacher).group_by(profession).execute())
...
print(query().select(profession_group).from_(persons).group_by(profession).execute())
...
print(query().select().from_(numbers).execute())

print(query().select().from_(numbers).group_by(parity).execute())
...
print(query().select().from_(numbers).group_by(parity, prime).execute())
...
print(query().select().from_(numbers).group_by(parity).having(odd).execute())
...
print(query().select().from_(numbers).order_by(descendent_compare).execute())
...
print(query().select().from_(numbers1, numbers2).execute())
